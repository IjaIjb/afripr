...
const PsychometricTestCourseAdmin = () => {
  ...
  const handleTagChange = (tagCode: string) => {
    setFormData((prevState: any) => {
      const isSelected = prevState.tags_associated.includes(tagCode);
      const updatedTags = isSelected
        ? prevState.tags_associated.filter((code: string) => code !== tagCode)
        : [...prevState.tags_associated, tagCode];
  ...
  const handleSkillAdd = () => {
    if (skillInput.trim() && !formData.skills_covered.includes(skillInput.trim())) {
      setFormData((prevState: any) => ({
        ...prevState,
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const response = await AdminApis.addPsychometricTestCourse(formattedData);
      console.log('Psychometric course added successfully:', response);
      if (response.data) {
        toast.success(response.data.message || "Psychometric course created successfully");
        navigate("/admin/psychometric-test-course");
      }
    } catch (error: any) {
      toast.error(error.message || "An error occurred");
      console.error('Error adding psychometric course:', error);
    } finally {
      setLoader(false);
    }
  };
    ...
    <AdminDashboardLayout>
      <div>
        <div className="mx-auto pb-10">
          <h2 className="text-[32px] font-bold text-primary mb-6">Add Psychometric Test Course</h2>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {formData.skills_covered.map((skill: string, index: number) => (
                  <span
                    key={index}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {availableTags.map((tag: any) => (
                    <label key={tag.id} className="flex items-center space-x-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={formData.tags_associated.includes(tag.code)}
            ...
            <div className="flex justify-center">
              ...
              >
                {loader ? <LoadingSpinner /> : "Create Psychometric Course"}
              </button>
            </div>
          </form>
        </div>
...
export default PsychometricTestCourseAdmin;